...
class ActionService {
    ...
    action(request) {
        switch (request.event) {
            case _enum_EventType__WEBPACK_IMPORTED_MODULE_0__.EventType.CONNECTION_REQUEST:
                return this.createPostRequest(request);
            case _enum_EventType__WEBPACK_IMPORTED_MODULE_0__.EventType.MESSAGE_REQUEST:
                if (this.dataset[request.message.uuid] == undefined) {
                    this.dataset[request.message.uuid] = true;
                    return this.createPostRequest(request);
                }
                return request.callback(true);
            case _enum_EventType__WEBPACK_IMPORTED_MODULE_0__.EventType.PROFILE_REQUEST:
                return this.createGetRequest(request);
            case _enum_EventType__WEBPACK_IMPORTED_MODULE_0__.EventType.USER_REQUEST:
                return this.createGetRequest(request);
            case _enum_EventType__WEBPACK_IMPORTED_MODULE_0__.EventType.VISIT_REQUEST:
                return this.navigate(request);
            case _enum_EventType__WEBPACK_IMPORTED_MODULE_0__.EventType.PREDICTION:
                return this.createGetRequest(request);
            default:
    ...
    navigate(request) {
        window.location.href = request.message.url;
    }
    createGetRequest(request) {
        axios__WEBPACK_IMPORTED_MODULE_1___default().get(request.message.url, {
            headers: request.message.service.headers
        }).then((result) => {
            request.callback(result.data);
        });
    }
    createPostRequest(request) {
        axios__WEBPACK_IMPORTED_MODULE_1___default().post(request.message.url, request.message.data, {
            headers: request.message.service.headers
        }).then((result) => {
            request.callback(result.data);
        });
...
class Runner {
    constructor() {
        ...
        this.messageService.monitorMessage();
    }
}
new Runner();
